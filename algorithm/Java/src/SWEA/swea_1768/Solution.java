...
class Solution {
    private final static int N              = 4;  // 숫자의 길이
    private final static int MAX_QUERYCOUNT = 1000000;

    private static int digits[] = new int[N]; 
    private static int digits_c[] = new int[10]; // index가 생각하는 수로 존재함 (0/1)
	...
	private static boolean isValid(int guess[]) {
		...
		for (int idx = 0; idx < N; ++idx) {
			if (guess[idx] < 0 || guess[idx] >= 10 || guess_c[guess[idx]] > 0) return false; // 잘못된 추측
			guess_c[guess[idx]]++;
		}
		return true;
	}
